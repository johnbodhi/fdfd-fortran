...
module global
	...
	character*500 result_filename, geometry_filename;
	character*500 materials_filename;

	real Sx, Sy, Sz, dx, dy, dz,frequency, w;
...
subroutine setCoefficients
    ...
    Cexhx = mu_xi/(j*w*dx*eps_xi); Cexhi = mu_xi/(j*w*dx*eps_xi);
    Ceyhy = mu_yi/(j*w*dy*eps_yi); Ceyhi = mu_yi/(j*w*dy*eps_yi);
    Cezhz = mu_zi/(j*w*dz*eps_zi); Cezhi = mu_zi/(j*w*dz*eps_zi);
     
    Chxex = eps_xi/(j*w*dx*mu_xi); Chxei = eps_xi/(j*w*dx*mu_xi); 
    Chyey = eps_yi/(j*w*dy*mu_yi); Chyei = eps_yi/(j*w*dy*mu_yi); 
    Chzez = eps_zi/(j*w*dz*mu_zi); Chzei = eps_zi/(j*w*dz*mu_zi);
...
!	use iflib
	...
	result_filename = "no_results_file";
...
subroutine initialize
	...
	phi = 0.0; forall (i=1:361) theta(i) = (i-1)*pi/100;
...
subroutine solveSparseMatrices
    ...
    integer lda, m, lwork, ldstrt
    complex*16, allocatable :: work(:);
    integer matvecprod, precondLeft, precondRight, dotProd
    complex*16 ONE, ZERO;
    REAL(8) elapsed_time
...
!-------------------------------------------------------------
    ...
    icntl(3) = 40;       ! Save the convergence history on standard output
    icntl(7) = number_of_iterations;     ! Maximum number of iterations
    icntl(4) = 0;       ! preconditioner location
    icntl(5) = 0;       ! orthogonalization scheme
    icntl(6) = 0;       ! initial guess
    icntl(8) = 1;       ! residual calculation strategy at restart
    cntl(1)  = tolerance; 


! Reverse communication implementation
10     call drive_zgmres(Nu,Nu,m,lwork,work,irc,icntl,cntl,info,rinfo)
       revcom = irc(1)
       colx   = irc(2)
       coly   = irc(3)
       colz   = irc(4)
       ...
       if (revcom.eq.matvecprod) then
! perform the matrix vector product
!        work(colz) <-- A * work(colx)

        call matvec(Nu,work(colx:colx+Nu-1),work(colx:colz+Nu-1))
         !call zgemv('N',Nu,Nu,ONE,a,lda,work(colx),1, ZERO,work(colz),1)
...
!         work(colz) <-- M^{-1} * work(colx)
         call zcopy(Nu,work(colx),1,work(colz),1)
         !call ztrsm('L','L','N','N',Nu,1,ONE,A,lda,work(colz),Nu)
         goto 10

       else if (revcom.eq.precondRight) then
! perform the right preconditioning
         call zcopy(Nu,work(colx),1,work(colz),1)
         !call ztrsm('L','U','N','N',Nu,1,ONE,A,lda,work(colz),Nu)
         goto 10
...
!      work(colz) <-- work(colx) work(coly)
         ...
         call zgemv('C',Nu,nbscal,ONE,work(colx),Nu,work(coly),1,ZERO,work(colz),1)
         goto 10
       endif
