...
C*
*
        subroutine drive_sgmres(n,nloc,m,lwork,work,
     &                         irc,icntl,cntl,info,rinfo)
*
*  Purpose
*  =======
*    drive_sgmres is the driver routine for solving the linear system 
...
* setup some pointers on the workspace
       ...
       rotCos   = rotSin + m
*
       call sgmres(nloc,m,work(bptr),work(xptr),
...
*
        subroutine sgmres(n,m,b,x,H,w,r0,V,dot,yCurrent,xCurrent,rotSin,
     &                   rotCos,irc,icntl,cntl,info,rinfo)
*
*
*  Purpose
*  =======
*  sgmres solves the linear system Ax = b using the
...
* ----------------------
       integer  irc(*)
       real x(*), H(m+1,*), w(*), r0(*) 
       real V(n,*),dot(*),yCurrent(*)  
       real xCurrent(*), rotSin(*)
       real rotCos(*)
*
* Local variables
* ---------------
...
*
*       Most outer loop : sgmres iteration
*
*       REPEAT
 7     continue
*
...
*
           ...
           call scopy(n,w,1,r0,1)
         endif
       endif
 46    continue
       if ((bea.le.cntl(1)).or.(iterOut*m+jH.ge.iterMax)) then
*
*        dnormres = snrm2(n,r0,1) 
*
         irc(1) = prosca
         irc(2) = r0ptr
         irc(3) = r0ptr
...
*
*
        subroutine init_sgmres(icntl,cntl)
*
*  Purpose
*  =======
